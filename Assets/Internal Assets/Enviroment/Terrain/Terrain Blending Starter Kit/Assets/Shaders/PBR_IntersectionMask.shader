...
Shader "Environment Starter/Standard Intersection Mask" {
	...
	SubShader {
		...
		half _IntersectionThresholdMaxNormal, _IntersectionPowNormal, _DepthFactorNormal, _InflateGround;
		...
		void vert(inout appdata_full v, out Input o)
		{
			...
			float blendMask = saturate(pow(saturate(_DepthFactor * o.worldSpacePosition.y) / _IntersectionThresholdMax, _IntersectionPow));
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			float diff = 1 - saturate(pow(saturate(_DepthFactor * IN.worldSpacePosition.y) / _IntersectionThresholdMax, _IntersectionPow));
